...
static char sPokebotRamTitle[48] = "RAM Bridge: OFF";
static char sPokebotInputTitle[48] = "Input Controller: OFF";

static void PokebotBridge_RefreshTitles(void)
{
    snprintf(sPokebotRamTitle, sizeof(sPokebotRamTitle), "RAM Bridge: %s", sPokebotRamEnabled ? "ON" : "OFF");
    snprintf(sPokebotInputTitle, sizeof(sPokebotInputTitle), "Input Controller: %s", sPokebotInputEnabled ? "ON" : "OFF");
}

static void PokebotBridge_ToggleRam(void)
{
    sPokebotRamEnabled = !sPokebotRamEnabled;
    PokebotBridge_RefreshTitles();
}
...
static void PokebotBridge_ShowStatus(void)
{
    ...
    do
    {
        ...
        Draw_DrawString(10, posY, COLOR_WHITE,
            "Foundation build only.\nBoth states return to OFF on reboot.\n\nPress B to go back.");
        Draw_FlushFramebuffer();
        Draw_Unlock();
    }
...
Menu pokebotBridgeMenu = {
    "Pokebot3DS Bridge",
    {
        { sPokebotRamTitle, METHOD, .method = &PokebotBridge_ToggleRam },
        { sPokebotInputTitle, METHOD, .method = &PokebotBridge_ToggleInput },
